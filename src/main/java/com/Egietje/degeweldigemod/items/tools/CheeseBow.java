...
public class CheeseBow extends ItemBow {
	
	public CheeseBow() {
		this.maxStackSize = 1;
		this.setMaxDamage(734);
		this.addPropertyOverride(new ResourceLocation("pull"), new IItemPropertyGetter() {
			...
			public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) {
				if (entityIn == null) {
					...
					return itemstack != null && itemstack.getItem() == CheeseItems.CHEESE_BOW ? (float) (stack.getMaxItemUseDuration() - entityIn.getItemInUseCount()) / 10.0F : 0.0F;
				}
			}
		});
