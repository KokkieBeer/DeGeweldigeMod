	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if (!entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn.isNonBoss()) {
			if (entityIn.timeUntilPortal > 0) {
				...
				if (!worldIn.isRemote && !pos.equals(CheeseUtils.getField(Entity.class, entityIn, "lastPortalPos"))) {
					CheeseUtils.setField(Entity.class, entityIn, "lastPortalPos", pos);
					...
					CheeseUtils.setField(Entity.class, entityIn, "lastPortalVex", new Vec3d(d1, d2, 0.0D));
					CheeseUtils.setField(Entity.class, entityIn, "teleportDirection", blockpattern$patternhelper.getForwards());
				}

				CheeseUtils.setField(Entity.class, entityIn, "inPortal", true);
